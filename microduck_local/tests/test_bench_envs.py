...
def test_ipc_floor_measures_the_plumbing_and_nothing_else():
    """The number that decides whether a rollout-loop rewrite is worth it.

    Measured on an 18-core M5 Max: 86 us per vec-step at 32 envs (k=2)
    against an 883 us REAL vec-step — the plumbing is ~10% and the other 90%
    is physics plus the memory contention of many concurrent mjData working
    sets. That is what ruled out replacing the SB3 rollout loop wholesale.
    """
...
def test_null_env_is_module_level_so_fork_children_can_rebuild_it():
    """A closure here would break the moment the backend pickles."""
    import pickle

    from microduck_local.bench_envs import _NullEnv

    assert pickle.loads(pickle.dumps(_NullEnv)) is _NullEnv
    env = _NullEnv()
    obs, _ = env.reset()
    assert obs.shape == _NullEnv.observation_space.shape
