...
class Profile:
    ...
    def envs_per_worker(self, n_envs: int) -> int:
        ...
        1 (one process per env) unless the profile packs, which only the
        Linux profile does and only when the fleet outnumbers the cores.
        Beyond that point the extra processes cannot run in parallel anyway,
        and each one costs the parent two semaphore operations per vec-step:
        at 32 envs on 4 cores, packing to 4 workers measured +7% throughput
        and halved vec-env startup (8.2 s -> 4.3 s). Packing is invisible to
        the caller — same obs/rew/done stream, pinned by
...
def build_profile(name: str | None = None, cores: int | None = None) -> Profile:
    ...
    if name == "mac":
        ...
        profile = Profile(
            ...
            rollout_threads=None, interop_threads=1, pack_workers=False,
        )
    elif name == "linux":
        profile = Profile(
